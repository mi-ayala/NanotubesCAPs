### Epsilon potential functions

function epsilon_energy(x_input, connectivity, b, θ, epsilon)
...
function epsilon_energy(x_input, connectivity, b::IntervalArithmetic.Interval, θ::IntervalArithmetic.Interval, epsilon::IntervalArithmetic.Interval)
...
function Grad_epsilon(x, connectivity, b, θ, epsilon)
    x = reshape(x, :, 3)
    ...
    ### Potential
    vBond = d -> sqrt((d - b)^2 + epsilon)
    vBondPrime = d -> (d - b) / sqrt((d - b)^2 + epsilon)

    vAngle = a -> sqrt((a - θ)^2 + epsilon)
    vAnglePrime = a -> (a - θ) / sqrt((a - 2pi / 3)^2 + epsilon)
...
function Grad_epsilon(x, connectivity, b::IntervalArithmetic.Interval, θ::IntervalArithmetic.Interval, epsilon::IntervalArithmetic.Interval)
...
function extended_Grad_epsilon(x_input, x_fix, connectivity, b, θ, epsilon)
    ...
    _, Fx = Energy_Grad_epsilon(x, connectivity, b, θ, epsilon)
...
function extended_Grad_epsilon(x_input, x_fix, connectivity, b::IntervalArithmetic.Interval, θ::IntervalArithmetic.Interval, epsilon::IntervalArithmetic.Interval)
...
function extended_Hess_epsilon(x_input, x_fix, connectivity, b, θ, epsilon)
    ...
    ### Computing Fx
    ### Computing vector field
    _, Fx = Energy_Grad_epsilon(reshape(x, :, 3), connectivity, b, θ, epsilon)
    ...
    ### Potential
    vBondPrime = d -> (d - b) / sqrt((d - b)^2 + epsilon)
    vBondPrimePrime = d -> 1 / sqrt((d - b)^2 + epsilon) - (d - b)^2 / ((d - b)^2 + epsilon)^(3 / 2)

    vAnglePrime = a -> (a - θ) / sqrt((a - θ)^2 + epsilon)
    vAnglePrimePrime = a -> 1 / sqrt((a - θ)^2 + epsilon) - (a - θ)^2 / ((a - θ)^2 + epsilon)^(3 / 2)
...
function extended_Hess_epsilon(x_input, x_fix, connectivity, b::IntervalArithmetic.Interval, θ::IntervalArithmetic.Interval, epsilon::IntervalArithmetic.Interval)
...
function Energy_Grad_epsilon(x, connectivity, b, θ, epsilon)
...
function Energy_Grad_epsilon(x, connectivity, b::IntervalArithmetic.Interval, θ::IntervalArithmetic.Interval, epsilon::IntervalArithmetic.Interval)
