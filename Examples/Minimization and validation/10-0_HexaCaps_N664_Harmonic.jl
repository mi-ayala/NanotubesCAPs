...
### A proof of the (10,0)-nanotube with hexagonal caps and N = 664 atoms using the harmonic potential. We minimize the energy and validate the simulation. All the other examples follow the same implementation structure.
...
numRings = 31
connectivity, x_initial = get_connectivity_10_0_hexagonal(numRings)
N = size(x_initial, 1)
...
r = get_proof(x_newton, F_int, DF_int, 9.99938970585269e-8)

# ### Save the data 
# save("data/10-0_HexaCaps_N664_Harmonic.jld2", "x_newton", x_newton, "r", r)

### The data is saved to be loaded and analyzed in the file `plots_5-5_HexaCaps_N664_Harmonic.jl`, where we generate plots and examine the results.